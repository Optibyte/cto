...
export class DashboardService {
  ...
  private calculateMetricKPI(metrics: Metric[], metricType: string) {
  ...
  async getTeamPerformance() {
    ...
    return teams.map((team) => {
      const velocityMetrics = team.metrics.filter(
        (m) => m.metricType === 'velocity',
      );
      const qualityMetrics = team.metrics.filter(
        (m) => m.metricType === 'quality',
      );
